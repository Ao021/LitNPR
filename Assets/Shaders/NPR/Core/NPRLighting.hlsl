...
#include "NPRGlobalIllumination.hlsl"
...
half3 LightingPhysicallyBased(BRDFData brdfData, BRDFData brdfDataClearCoat,half3 lightColor, half3 lightDirectionWS, half lightAttenuation,half3 normalWS, half3 viewDirectionWS,half clearCoatMask, bool specularHighlightsOff, half thick, half3 anisohair)
{
    half NdotL = dot(normalWS, lightDirectionWS);
    half SmoothNoL = smoothstep(_NdotLOffset,_NdotLOffset+_LDSmooth,NdotL);
    half3 radiance = lightColor * (lightAttenuation * lerp(_DColor,_LColor,SmoothNoL));
#ifdef _SSS_ON
    half SSS = smoothstep(_SSSOffset-_SSSWidth,_SSSWidth,NdotL) * smoothstep(_SSSWidth,_SSSOffset-_SSSWidth,NdotL);
    radiance += _SSSScale * lightColor * lightAttenuation * SSS * _InnerColor;
#endif
    half3 brdf = brdfData.diffuse;
#ifndef _SPECULARHIGHLIGHTS_OFF
    [branch] if (!specularHighlightsOff)
    {
        half spec = DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS);

        #ifdef _ANISOHAIR_ON
        half anisoHairFresnel = pow(1-saturate(dot(normalWS,viewDirectionWS)),_AnisoHairFresnelPow) * _AnisoHairFresnelIntensity;
        half anisoHair = saturate(1-anisoHairFresnel)*anisohair;
        brdf += anisoHair;
        #endif

        
        brdf += brdfData.specular * 10 *step(0.1,spec) *  pow((1-brdfData.roughness),10);
...
#ifdef _TRANS_ON
    float3 H = normalize(lightDirectionWS + normalWS * _Distortion);
    float transDot = pow(saturate(dot(viewDirectionWS, -H)), _Power);
    half3 transRadiance = _TransIndensity* pow((1-thick), _ThickScale) * lightColor * lightAttenuation * _SubColor;
    finalColor += transDot * transRadiance;
#endif
...
half3 LightingPhysicallyBased(BRDFData brdfData, BRDFData brdfDataClearCoat, Light light, half3 normalWS, half3 viewDirectionWS, half clearCoatMask, bool specularHighlightsOff,half thick, half3 anisohair)
{
    return LightingPhysicallyBased(brdfData, brdfDataClearCoat, light.color, light.direction, light.distanceAttenuation * light.shadowAttenuation, normalWS, viewDirectionWS, clearCoatMask, specularHighlightsOff,thick,anisohair);
}
...
half3 LightingPhysicallyBased(BRDFData brdfData, Light light, half3 normalWS, half3 viewDirectionWS)
{
    ...
    return LightingPhysicallyBased(brdfData, noClearCoat, light, normalWS, viewDirectionWS, 0.0, specularHighlightsOff,1,1);
}
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData)
{
    ...
    half3 anisohair = 0;
    #ifdef _ANISOHAIR_ON
    anisohair = surfaceData.anisohair;
    #endif
    ...
    if (IsMatchingLightLayer(mainLight.layerMask, meshRenderingLayers))
    {
        lightingData.mainLightColor = LightingPhysicallyBased(brdfData, brdfDataClearCoat,
                                                              mainLight,
                                                              inputData.normalWS, inputData.viewDirectionWS,
                                                              surfaceData.clearCoatMask, specularHighlightsOff, surfaceData.thickness,anisohair);
    }
