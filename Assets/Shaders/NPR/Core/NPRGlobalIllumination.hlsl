...
{
    //bakedGI = step(0.1,bakedGI);
    
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = saturate(dot(normalWS, viewDirectionWS));

    //环境光高光
    half fresnelTerm = step(0.5 ,Pow4(1.0 - NoV));
    half GIDiffuse = GetIllumination(bakedGI);
    half3 GIColor =  bakedGI/ GIDiffuse;
    GIDiffuse = (smoothstep(_GIStep,_GIStep+_GIStepSmooth,GIDiffuse)+_GIMin)/(1+_GIMin);
    
    half3 indirectDiffuse =  GIDiffuse * GIColor;
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfData.perceptualRoughness, 1.0h);

    half SpecIndensity = HALF_EPS + indirectSpecular/(HALF_EPS + GetIllumination(indirectSpecular));
    half3 SpecColor = indirectSpecular/SpecIndensity;

#ifdef _STEPSPECGI
    indirectSpecular = pow( floor( _SpecColorNum*SpecIndensity)/ _SpecColorNum,_SpecIndensity) * SpecColor;
#endif
    
    
    half3 color = _GIScale * EnvironmentBRDF(brdfData, indirectDiffuse, indirectSpecular, fresnelTerm);


    //color = pow( floor(2*SpecIndensity)/2,3) * SpecColor ;
