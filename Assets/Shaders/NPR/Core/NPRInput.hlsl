...
//---------------------------------各向异性高光-------------------------------------------
half _AnisoHairFresnelPow;
half _AnisoHairFresnelIntensity;

//---------------------------------边缘光-------------------------------------------
half _RimWidth;  
half _RimThreshold;
half4 _RimColor;
half  _RimSmoothThreshold;
half _RimStep;
half _RimStepSmooth;
//---------------------------------面部-------------------------------------------
half  _FaceNormalRate;
half  _FaceCenterOffset;

//---------------------------------透射-------------------------------------------
half _TransIndensity;
half _Distortion;
half _Power;
half3 _SubColor;
half  _ThickScale;
...
{
#ifdef _ANISOHAIR_ON
    outSurfaceData.anisohair = SAMPLE_TEXTURE2D(_AnisoMask,sampler_AnisoMask,uv);
#endif
    
    
    outSurfaceData.thickness = SAMPLE_TEXTURE2D(_ThickTex,sampler_ThickTex,uv);
...
half GetIllumination(half4 color)
{
    return 0.299* color.r + 0.587*color.g + 0.114*color.b;
}
half GetIllumination(half3 color)
{
    return 0.299* color.r + 0.587*color.g + 0.114*color.b;
}